function [R,T,eRMSD,Sum_Square,Sort_Sum_Square] = CoordiExam(fromXYZ, toXYZ)
    ...
    Sum_Square=sum((toXYZ-newXYZ).^2,2);
    Sort_Sum_Square=sort(Sum_Square);

    eRMSD=sqrt(sum(Sum_Square)/len_2);
end