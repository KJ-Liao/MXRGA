...
parfor r=1:size(Sample,2)
    ...
    PDB_CA_Idx=strcmp({PDB.Model.Atom.AtomName}, 'CA')';
    Asy_Unit=PDB_Model(PDB_CA_Idx,:);

    % Sequence
    PDB_Seq=aminolookup(strrep(strrep([PDB.Model.Atom(PDB_CA_Idx).resName], 'SEC', 'CYS'), 'UNK', 'GLY'));

    % Pack the Set of Symmertic Unit
    Unit=[]; SMTRY_Unit=[];
    for op=2:size(Operator,1)/3
        SMTRY_Unit=Asy_Unit*[Operator(3*op-2,1:3);Operator(3*op-1,1:3);Operator(3*op,1:3)]'+[Operator(3*op-2,4),Operator(3*op-1,4),Operator(3*op,4)];
    ...
    for x=-3:3
        for y=-3:3
            for z=-3:3
                if x~=0||y~=0||z~=0
                    NB_Unit=Unit+[x*Unit_Cell(1), y*Unit_Cell(2), z*Unit_Cell(3)];
