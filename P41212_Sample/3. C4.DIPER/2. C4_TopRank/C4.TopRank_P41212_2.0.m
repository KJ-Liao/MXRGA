...
C4_TopRank_Result(1).Lowest_QC4_RMSD_95=[];
...
parfor r=1:size(Result,2)
    ...
    if ~isempty(Qualified_Idx)
        for QI=1:size(Qualified_Idx,1)
            ...
            % Extract Coordinates of Alpha Carbon (CA)
            HMD_ID=[Result(r).ID, '.002.', num2str(C2_TopRank_Result(r).Output_Idx(Result(r).Linear_Table(rp,2)))];
...
for r=1:size(Result,2)
    ...
    if ~isempty(Qualified_Idx)
        for QI=1:size(Qualified_Idx,1)
            rp=Qualified_Idx(QI,1);
            t=t+1;
            Sum_Table(t).ID=[Result(r).ID, '.002.', num2str(C2_TopRank_Result(r).Output_Idx(Result(r).Linear_Table(rp,2))),'.000.',num2str(Result(r).Linear_Table(rp,4))];
